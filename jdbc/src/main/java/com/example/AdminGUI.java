...
public class AdminGUI extends JFrame {
	...
	private static void sendDataToUnity(String id) {
		String[] orderData = JSS.getOrderById(id);
		System.out.println("\n√  유니티로 데이터를 전송합니다 - " + orderData[3] + ", " + orderData[4]);
		String shape = orderData[3];
		String custom = orderData[4];

		String shapeToUnity = "";
		String customToUnity = "";
		...
		if ("Square".equals(shape)) {
			...
			System.out.println("데이터 오류 - 허용되지 않은 데이터 (도형)");
		}

		if ("NO".equals(custom)) {
			customToUnity = "f";
		} else if ("YES".equals(custom)) {
			customToUnity = "t";
		} else {
			System.out.println("데이터 오류 - 허용되지 않은 데이터 (스탬프)");
		}

		try {
			Socket socket = new Socket("127.0.0.1", 9999);
			PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
			out.println(shapeToUnity + " " + customToUnity);
			socket.close();
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("오류가 발생했습니다 - 유니티 데이터 전송 실패");
		}
	}
		...
		startBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (selected_order.getValueAt(0, 1) == null) {
					...
					int answer = JOptionPane.showConfirmDialog(null, "공정을 시작하시겠습니까?", "공정 시작",
							JOptionPane.YES_NO_OPTION);
					if (answer == JOptionPane.YES_OPTION) {
						int targetOrder = Integer.parseInt(String.valueOf(selected_order.getValueAt(0, 1)));
						sendDataToUnity(String.valueOf(targetOrder));
